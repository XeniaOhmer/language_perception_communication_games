...
class ModelTrainer:
    def __init__(self, model, train_input,
        ...
        if sfunc is not None and isinstance(train_input, tuple):
            ...
            if verbose:
                max_act = np.max(train_y)
                mean_max = np.mean(np.max(train_y, axis=1))
                mean = np.mean(train_y)
                print('sf={}, max_act={:.02f}, meanmax_act={:.02f}, mean_act={:.04f}'.format(func_args['factor'],
                                                                                             max_act,
                                                                                             mean_max,
                                                                                             mean))
    ...
    def evaluate_model(self, test_x, test_y, **args):
        if args is not None:
            ...
            if type(self.class_names) is list and len(self.class_names) != predictions.shape[-1]:
                print('specified class names do not match output shape, set to None')
